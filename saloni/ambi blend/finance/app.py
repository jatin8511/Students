...
def buy():
    ...
    if request.method == "POST":
        ...
        #Ensure shares is a valid input
        ...
        #Ensure there is enough money to buy
        if cash < total_cost:
...
def register():
    ...
    if request.method == "POST":
        ...
        # Ensure username was submitted
        if not username:
            return apology("must type in username", 400)

        # Ensure password was submitted
        elif not password:
            return apology ("must type in password", 400)
        ...
        # Remember which user has logged in aka cookie
        session["user_ id"] = rows[0]["id"]
